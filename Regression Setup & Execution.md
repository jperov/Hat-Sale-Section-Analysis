...
## Purpose
Our main goal was to determine if there was a statistically significant decrease in company-wide sale hat revenue in the period following the pricing change. Regression is a statistical technique used to determine how much an outcome variable changes as a result of changes in one or more input variables. In our case, we wanted to find out how much the pricing change affected sale hat revenue while controlling for seasonality and time trends to determine if there truly was a statistical decrease in revenue after the change occurred.
...
2. We then created a SQL query to gather the variables used in the regression model. Fields made and their purposes are outlined below:

    1. **Date:**  Each day from January 2024 through 2025.

    2. **Net sales:**  Total sale hat section revenue on that day.

    3. **Time index:** Counted the number of passing days starting from 1 on January 1st 2024 counting through 2025. This variable was used in our regression to control for how much sales naturally increase or decrease per day over time independent of monthly seasonality and the pricing change. 
    ...
    5. **Month names (Jan - Dec):** Using a similar principle as the after period field, we made fields for each month of the year and assigned a 1 to sales values that occurred on days in that month in both 2024 and 2025, and a 0 to days that didn't occur in that month. These fields were created so our model could control for recurring monthly seasonality in sales. This means our 'after period' coefficient was not impacted by the usual increase in sales seen in November and December. March was omitted from our month fields to serve as a baseline under normal conditions.
...
6. **To perform the Cochrane–Orcut procedure we took the following steps:**
    ...
    2. Then for each individual column in our original input (Y = Net Sales, X = Variables) we used the slope coefficient in a formula to transform each column's values so the regression model runs on variables that are uncorrelated.
...
```excel
  ...
  X2 = the second value of selected X variable 
  X1 = the first value of selected X variable
```
...
This was the series of steps taken to arrive at our conclusion of average daily sales in the sale hat section decreasing by **$1074** following the sale hat pricing change, providing further evidence that the shift to flat-priced sections drove the decline in revenue.
